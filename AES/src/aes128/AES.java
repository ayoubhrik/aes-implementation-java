package aes128;


import javax.xml.bind.DatatypeConverter;

import java.io.*;

public class AES {

	final static int[] sBox = {};

	final static int[] inverseSbox = {};

	final static int[] LogTable = {
		0,   0,  25,   1,  50,   2,  26, 198,  75, 199,  27, 104,  51, 238, 223,   3, 
		100,   4, 224,  14,  52, 141, 129, 239,  76, 113,   8, 200, 248, 105,  28, 193, 
		125, 194,  29, 181, 249, 185,  39, 106,  77, 228, 166, 114, 154, 201,   9, 120, 
		101,  47, 138,   5,  33,  15, 225,  36,  18, 240, 130,  69,  53, 147, 218, 142, 
		150, 143, 219, 189,  54, 208, 206, 148,  19,  92, 210, 241,  64,  70, 131,  56, 
		102, 221, 253,  48, 191,   6, 139,  98, 179,  37, 226, 152,  34, 136, 145,  16, 
		126, 110,  72, 195, 163, 182,  30,  66,  58, 107,  40,  84, 250, 133,  61, 186, 
		43, 121,  10,  21, 155, 159,  94, 202,  78, 212, 172, 229, 243, 115, 167,  87, 
		175,  88, 168,  80, 244, 234, 214, 116,  79, 174, 233, 213, 231, 230, 173, 232, 
		44, 215, 117, 122, 235,  22,  11, 245,  89, 203,  95, 176, 156, 169,  81, 160, 
		127,  12, 246, 111,  23, 196,  73, 236, 216,  67,  31,  45, 164, 118, 123, 183, 
		204, 187,  62,  90, 251,  96, 177, 134,  59,  82, 161, 108, 170,  85,  41, 157, 
		151, 178, 135, 144,  97, 190, 220, 252, 188, 149, 207, 205,  55,  63,  91, 209, 
		83,  57, 132,  60,  65, 162, 109,  71,  20,  42, 158,  93,  86, 242, 211, 171, 
		68,  17, 146, 217,  35,  32,  46, 137, 180, 124, 184,  38, 119, 153, 227, 165, 
		103,  74, 237, 222, 197,  49, 254,  24,  13,  99, 140, 128, 192, 247, 112,   7};

	final static int[] AntiLogTable = {
		1,   3,   5,  15,  17,  51,  85, 255,  26,  46, 114, 150, 161, 248,  19,  53, 
		95, 225,  56,  72, 216, 115, 149, 164, 247,   2,   6,  10,  30,  34, 102, 170, 
		229,  52,  92, 228,  55,  89, 235,  38, 106, 190, 217, 112, 144, 171, 230,  49, 
		83, 245,   4,  12,  20,  60,  68, 204,  79, 209, 104, 184, 211, 110, 178, 205, 
		76, 212, 103, 169, 224,  59,  77, 215,  98, 166, 241,   8,  24,  40, 120, 136, 
		131, 158, 185, 208, 107, 189, 220, 127, 129, 152, 179, 206,  73, 219, 118, 154, 
		181, 196,  87, 249,  16,  48,  80, 240,  11,  29,  39, 105, 187, 214,  97, 163, 
		254,  25,  43, 125, 135, 146, 173, 236,  47, 113, 147, 174, 233,  32,  96, 160, 
		251,  22,  58,  78, 210, 109, 183, 194,  93, 231,  50,  86, 250,  21,  63,  65, 
		195,  94, 226,  61,  71, 201,  64, 192,  91, 237,  44, 116, 156, 191, 218, 117, 
		159, 186, 213, 100, 172, 239,  42, 126, 130, 157, 188, 223, 122, 142, 137, 128, 
		155, 182, 193,  88, 232,  35, 101, 175, 234,  37, 111, 177, 200,  67, 197,  84, 
		252,  31,  33,  99, 165, 244,   7,   9,  27,  45, 119, 153, 176, 203,  70, 202, 
		69, 207,  74, 222, 121, 139, 134, 145, 168, 227,  62,  66, 198,  81, 243,  14, 
		18,  54,  90, 238,  41, 123, 141, 140, 143, 138, 133, 148, 167, 242,  13,  23, 
		57,  75, 221, 124, 132, 151, 162, 253,  28,  36, 108, 180, 199,  82, 246,   1};

	final static int[][] rcon = {{0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36},
								{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
								{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
								{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}};



	final static int nbrOfRouds = 10;	

	private byte[][] keysMatrix;	
	private byte[][] plainTextMatrix;	

	public AES() {
		plainTextMatrix = new byte[4][4];
		keysMatrix = new byte[4][4*(nbrOfRouds+1)];
	}


	public static String completeWithZeros(String str) {
		int length = str.length();
		while(length < 32) {
			str += "0";
			length++;
		}
		return str;
	}

	
	private void generatePlaintextMatrix(String input) {
		String sb;
		byte b;
		int start = 0;
		int end = 2;

		for (int i = 0; i < plainTextMatrix.length; i++) {
			for (int j = 0; j < plainTextMatrix[0].length; j++) {
				sb = input.substring(start, end);
				plainTextMatrix[j][i] = (byte)(Integer.parseInt(sb.toString(), 16));
				start += 2;
				end += 2;
			}
		}
	}

	private void generateKeysMatrix(String key) {

		String sb;
		byte b;
		int start = 0;
		int end = 2;

		for (int i = 0; i < keysMatrix.length; i++) {
			for (int j = 0; j < keysMatrix[0].length/(nbrOfRouds+1); j++) {
				sb = key.substring(start, end);
				keysMatrix[j][i] = (byte)(Integer.parseInt(sb.toString(), 16));
				start += 2;
				end += 2;
			}
		}

		
		int rconIndex = 0;
		for (int i = 4; i < 4*(nbrOfRouds+1); i++) {
			byte[] col = getColumn(keysMatrix, i-1);
			if (i % 4 == 0) {
				
				col = rotateWord(col);
				col = subBytesKey(col);
				col = xorBytes(col, getColumn(keysMatrix, i-4));
				col = xorRcon(col, rconIndex);
				rconIndex++;
			} else {
				
				col = xorBytes(col, getColumn(keysMatrix, i-4));
			}
		
		columnCopy(keysMatrix, i, col);
		}
	}


	private byte[] getColumn(byte[][] mat, int pos) {
		byte[] result = new byte[mat.length];
		for (int i = 0; i < 4; i++)
			result[i] = mat[i][pos];
		return result;
	}


	private void columnCopy(byte[][] mat, int pos, byte[] col) {
		for (int i = 0; i < col.length; i++) {
			mat[i][pos] = col[i];
		}
	}

	
	private byte[] rotateWord(byte[] arr) {
		byte[] result = new byte[arr.length];
		for (int i = 0; i < arr.length-1; i++)
			result[i] = arr[i+1];
		result[result.length-1] = arr[0];
		return result;
	}

	
	private byte[] subBytesKey(byte[] arr) {
		byte[] result = new byte[arr.length];
		for (int i = 0; i < arr.length; i++) {
			int j = arr[i];
			result[i] = (byte)(sBox[j & 0x000000FF] & 0xFF);
		}
		return result;
	}


	private byte[] xorBytes(byte[] m, byte[] n) {
		byte[] result = new byte[m.length];
		for (int i = 0; i < m.length; i++)
			result[i] = (byte)(m[i] ^ n[i]);
		return result;
	}

	
	private byte[] xorRcon(byte[] arr, int index) {

		arr[0] = (byte)(arr[0] ^ (rcon[0][index] & 0xff));
		return arr;
	}

	
	private void subBytes() {
		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < 4; j++) {
				byte b = plainTextMatrix[i][j];
				plainTextMatrix[i][j] = (byte)(sBox[b & 0x000000FF] & 0xFF);
			}
		}
	}


	private void shiftRows() {
		byte[] row = new byte[4];
		int shift = 0;
		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < 4; j++) {
				row[j] = plainTextMatrix[i][j];
			}
			for (int j = 0; j < 4; j++) {
				plainTextMatrix[i][j] = row[(j+shift) % 4];
			}
			shift++;
		}
	}

	private void shiftRowsInverse() {
		byte[] row = new byte[4];
		int shift = 0;
		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < 4; j++) {
				row[(j+shift)%4] = plainTextMatrix[i][j];
			}
			for (int j = 0; j < 4; j++) {
				plainTextMatrix[i][j] = row[j];
			}
			shift++;
		}
	}

	private byte mul (int a, byte b) {
		int inda = (a < 0) ? (a + 256) : a;
		int indb = (b < 0) ? (b + 256) : b;

		if ( (a != 0) && (b != 0) ) {
			int index = (LogTable[inda] + LogTable[indb]);
			byte val = (byte)(AntiLogTable[ index % 255 ] );
			return val;
		}
		else 
			return 0;
    }


	private void mixColumns() {
		for (int c = 0; c < 4; c++) {
			byte a[] = new byte[4];
			
			for (int i = 0; i < 4; i++) { 
				a[i] = plainTextMatrix[i][c];
			}
		
			plainTextMatrix[0][c] = (byte)(mul(2,a[0]) ^ a[2] ^ a[3] ^ mul(3,a[1]));
			plainTextMatrix[1][c] = (byte)(mul(2,a[1]) ^ a[3] ^ a[0] ^ mul(3,a[2]));
			plainTextMatrix[2][c] = (byte)(mul(2,a[2]) ^ a[0] ^ a[1] ^ mul(3,a[3]));
			plainTextMatrix[3][c] = (byte)(mul(2,a[3]) ^ a[1] ^ a[2] ^ mul(3,a[0]));
		}
			
	}


	private void mixColumnsInverse() {
		for (int c = 0; c < 4; c++) {
			byte a[] = new byte[4];
			
			for (int i = 0; i < 4; i++) {
				a[i] = plainTextMatrix[i][c];
			}

			plainTextMatrix[0][c] = (byte)(mul(0xE,a[0]) ^ mul(0xB,a[1]) ^ mul(0xD, a[2]) ^ mul(0x9,a[3]));
			plainTextMatrix[1][c] = (byte)(mul(0xE,a[1]) ^ mul(0xB,a[2]) ^ mul(0xD, a[3]) ^ mul(0x9,a[0]));
			plainTextMatrix[2][c] = (byte)(mul(0xE,a[2]) ^ mul(0xB,a[3]) ^ mul(0xD, a[0]) ^ mul(0x9,a[1]));
			plainTextMatrix[3][c] = (byte)(mul(0xE,a[3]) ^ mul(0xB,a[0]) ^ mul(0xD, a[1]) ^ mul(0x9,a[2]));
		}
			
	}


	private void addRoundKey(int currentRound) {
		byte[] col;
		int j = 0;
		for (int i = 0; i < 4; i++) {
			col = xorBytes(getColumn(plainTextMatrix, i), getColumn(keysMatrix, currentRound*4+j));
			columnCopy(plainTextMatrix, i, col);
			j++;
		}
	}


	private void subBytesInverse() {
		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < 4; j++) {
				byte b = plainTextMatrix[i][j];
				plainTextMatrix[i][j] = (byte)(inverseSbox[b & 0x000000FF] & 0xFF);
			}
		}
	}


	/*
	 * Encrypt
	 */
	public String encrypt(String plaintext, String keytext) throws IOException {
		keytext = completeWithZeros(keytext);
		generateKeysMatrix(keytext);
		plaintext = completeWithZeros(plaintext);
		generatePlaintextMatrix(plaintext);

		addRoundKey(0);	

		for (int i = 1; i < nbrOfRouds+1; i++) {
			subBytes();
			shiftRows();
			if (i != nbrOfRouds)	
				mixColumns();
			addRoundKey(i);
		}
		
		for (int i = 0; i < 40; i++) {
			for (int j = 0; j <  4; j++) {
				System.out.print(keysMatrix[j][i]+" ");				
			}
			System.out.println("");
		}
		System.out.println(keysMatrix[1][40]);
		return (plaintextMatrixToString());
	}


	/*
	 * Decrypt
	 */
	public String decrypt(String plaintext, String keytext) throws IOException {
		keytext = completeWithZeros(keytext);
		generateKeysMatrix(keytext);
		plaintext = completeWithZeros(plaintext);
		generatePlaintextMatrix(plaintext);

		addRoundKey(nbrOfRouds);
		for (int i = nbrOfRouds-1; i > -1; i--) {
			subBytesInverse();
			shiftRowsInverse();
			addRoundKey(i);
			if (i != 0)	
				mixColumnsInverse();
		}
		return (plaintextMatrixToString());
	}


	private String plaintextMatrixToString() {
		StringBuilder s = new StringBuilder();
		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < 4; j++) {
				s.append(String.format("%02X", plainTextMatrix[j][i]));
			}
		}
		return s.toString();
	}


	public String bytesToHex(byte[] bytes) {
	    return DatatypeConverter.printHexBinary(bytes);
	}
	 
	public byte[] hexToBytes(String hexString) {
	    return DatatypeConverter.parseHexBinary(hexString);
	}
	
}